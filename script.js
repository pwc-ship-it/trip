...
/* ── 상태 ── */
var S={filterSite:'all',showHidden:false,groups:[],sites:[],projects:[],schedules:[],events:[],workTasks:[]};

function deepCopy(o){return JSON.parse(JSON.stringify(o));}
function normDate(s){
  // 어떤 형식이든 YYYY-MM-DD로 정규화  if(!s)return '';
  // 이미 YYYY-MM-DD 형식이면 그대로  if(/^\d{4}-\d{2}-\d{2}$/.test(String(s).trim()))return String(s).trim();
  // Date 객체 문자열 등 다른 형식은 파싱 후 변환  var d=new Date(s);
  if(isNaN(d.getTime()))return s;
  return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');
}
function loadData(){
  // localStorage 캐시 있으면 우선 사용, 없으면DEF
  try{
  ...
  // 캐시 없으면DEF 기본 데이터  var def=deepCopy(DEF);
...
function saveData(){
  // 1. 利됱떆 localStorage 캐시 ?낅뜲?댄듃 (?덈줈怨좎묠 일理쒖떊 상태 蹂댁옣)
  saveCache({groups:S.groups,sites:S.sites,projects:S.projects,schedules:S.schedules,events:S.events,workTasks:S.workTasks});
  // 2. 鍮꾨룞湲곕줈 Sheets에 저장  var url=getSheetsUrl();
    ...
    .then(function(data){
      ...
      if(data.schedules&&data.schedules.length){
        S.schedules=data.schedules.map(function(sc){
          sc.start=normDate(sc.start);sc.end=normDate(sc.end);
          // Sheets에서 hidden일"TRUE"/"FALSE" 문자열로 오는 경우 변환          if(typeof sc.hidden === 'string'){
            sc.hidden = sc.hidden.toUpperCase()==='TRUE'||sc.hidden==='1'||sc.hidden==='true';
          }
          return sc;
        });
      ...
      // 성공적으로 로드된 데이터를 캐시에 저장      saveCache({groups:S.groups,sites:S.sites,projects:S.projects,schedules:S.schedules,events:S.events,workTasks:S.workTasks});
...
function renderSidebar(){
  ...
  // 그룹별  var groups=S.groups&&S.groups.length?S.groups:[{id:'_none',name:'사이트'}];
...
function addEv(el,evts,chips){
  ...
  // 겹침 처리: 칩들을px 기준 정렬 후 가까운 것은 너비 제한
  if(chips){
...
function addBar(el,sched){
  ...
  var txt=dr+' 쨌 '+sched.name+' ['+tl+'] ('+days+'일'+(sched.note?' 쨌 '+sched.note:'');
...
function addWtBar(el,wt){
  ...
  var txt=dr+' 쨌 '+wt.title+(wt.note?' ('+wt.note+')':'')+'  '+days+'일';
...
/* ?? 간트 렌더 ?? */
/* ?묒뾽 ?덉씤 諛곗젙: 寃뱀튂일?묒뾽? 蹂꾨룄 ?됱쑝濡?遺꾨━ */
function assignWtLanes(wts){
  // wts: [{start,end,...}] 일媛?wt일lane 踰덊샇 遺일  var lanes=[]; // lanes[i] = 留덉?留됱쑝濡일앸궃 날짜
  wts.forEach(function(wt){
    var assigned=false;
    for(var i=0;i<lanes.length;i++){
      if(wt.start>lanes[i]){ // 일?덉씤일?ㅼ뼱媛일일덉쓬
...
function renderGantt(){
  ...
  projs.forEach(function(proj){
    ...
    if(wts.length){
      ...
      var wsf=document.createElement('div');wsf.className='wksecfix';wsf.innerHTML='<span class="wksecfix-lbl">일?묒뾽</span>';
      ...
      for(var li=0;li<wtLanes;li++){
        ...
        // ?덉씤 ?덉씠釉? 泥?踰덉㎏ ?묒뾽 ?쒕ぉ ?쒖떆 (?щ윭 媛쒕㈃ ?レ옄)
        var laneLabel=laneTasks.length===1?laneTasks[0].title:(li+1)+'踰일덉씤 ('+laneTasks.length+'嫄?';
    ...
    // 출장 행    var tasks=[];scheds.forEach(function(s){if(tasks.indexOf(s.task)<0)tasks.push(s.task);});
    tasks.forEach(function(task){
      scheds.filter(function(s){return s.task===task;}).forEach(function(sched,idx){
        ...
        gf2.innerHTML='<div class="gtask">'+(idx===0?task:'')+'</div>'
          ...
          +'<span class="gbadge">'+dr+' 쨌 '+days+'일/span></div>';
...
function showEM(ex, exWt){
  ...
  var html='<div class="mtit">'+(ie?'이벤트/ ?묒뾽 ?섏젙':'이벤트/ ?묒뾽 등록')+'</div>';
  // ?좏삎 일  html+='<div class="ev-type-sel">'
    +'<button class="ev-type-btn'+(initType==='event'?' on':'')+'" id="evtab_event" onclick="switchEvTab(\'event\')">★ 주요 이벤트/button>'
    +'<button class="ev-type-btn'+(initType==='work'?' on':'')+'" id="evtab_work" onclick="switchEvTab(\'work\')">일?묒뾽</button>'
    +'</div>';
  ...
  html+='<div class="fg"><label class="fl">이벤트 제목</label><input type="text" id="fe_title" value="'+(ex?ex.title:'')+'" placeholder="예: 양산시작, 怨좉컼 諛⑸Ц"></div>';
  ...
  var wtDateInfo=exWt?(fmtFull(wt.start)+' 일'+fmtFull(wt.end)):'';
  html+='<div id="fe_work_sec" style="display:'+(initType==='work'?'block':'none')+'">';
  html+='<div class="fg"><label class="fl">?묒뾽 ?쒕ぉ</label><input type="text" id="fw_title" value="'+(exWt?wt.title:'')+'" placeholder="일 비전 ?뚮씪誘명꽣 理쒖쟻일 ?뚮엺 ?먭?"></div>';
  html+='<div class="fr">'
    ...
    +'<div class="fg"><label class="fl">醫낅즺일(YYYY-MM-DD)</label><input type="text" id="fw_end" value="'+(exWt?wt.end:todayStr)+'" maxlength="10" oninput="fmtDateInput(this);calcWD()" style="font-family:monospace;letter-spacing:1px"></div>'
    +'</div>';
  html+='<div class="dbox"><span id="fw_datebox" style="color:#ccc">'+wtDateInfo+'</span>'+(wtDateInfo?' &nbsp; ':'')+'湲곌컙: <span id="fw_days" style="font-weight:500">'+wtDays+'</span></div>';
  html+='<div class="fg"><label class="fl">?대떦일/ 비고</label><input type="text" id="fw_note" value="'+(exWt?wt.note:'')+'" placeholder="일 김성민, 일?쇱씤 ?먭?"></div>';
  // ?묒뾽 색상
...
function calcWD(){
  ...
  if(s&&e&&re.test(s)&&re.test(e)&&s<=e){
    ...
    var db=document.getElementById('fw_datebox');if(db)db.textContent=fmtFull(s)+' 일'+fmtFull(e);
  }
}
...
function saveWt(exId){
  ...
  if(start>end){alert('醫낅즺?쇱씠 시작?쇰낫일鍮좊쫭?덈떎.');return;}
  if(exId){
...
function delWt(id){if(!confirm('?묒뾽일삭제?좉퉴일'))return;S.workTasks=S.workTasks.filter(function(w){return w.id!==id;});saveData();cm();renderAll();}
...
function showSM(ex){
  ...
  var dateInfo=ie?(fmtFull(ex.start)+' 일'+fmtFull(ex.end)):'';
...
function fmtDateInput(el){
  // 숫자만 남기고 자동으로 - 삽입: 20260401 일2026-04-01
...
function calcD(){
  ...
  if(s&&e&&re.test(s)&&re.test(e)){
    if(s<=e){
      ...
      if(db)db.textContent=fmtFull(s)+' 일'+fmtFull(e);
...
function buildGrpSiteRows(){
  ...
  S.groups.forEach(function(grp){
    ...
    // 사이트 없을 때    if(!grpSites.length){var empty=document.createElement('div');empty.style.cssText='font-size:10px;color:#555;padding:3px 8px';empty.textContent='(사이트 없음)';el.appendChild(empty);}
    // 그룹 간 간격
...
/* ?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧
   ?몄썝 출장자愿由일먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧?먥븧 */

var REGION_AMERICAS_IDS=['ESHD','ESMI','ESHG','MILS','ESOT','UC2','?꾨?JV','BOSK_TN'];
...
function aggregatePersonTrips(){
  ...
  S.schedules.forEach(function(sc){
    // ?몄썝 출장자일? 숨김 ?щ일 臾닿일섍쾶 紐⑤뱺 ?쇱젙 吏묎퀎
    ...
    // 異쒖옣 ?먮옒 type 湲곕줉 (?몄썝일蹂듭닔 저장?덉쓣 일?덉쓬)
...
// rolling 12M 湲곗? 吏일퀎 출장자(以묐났 날짜 ?쒓굅)
...
// 留덉?留?복귀일湲곗? ?쒓뎅 체류일怨꾩궛
// 媛일理쒓렐 異쒖옣 醫낅즺일복귀일 ?ㅼ쓬?좊일일ㅻ뒛源뚯일일쇱닔
function calcKoreaDays(trips){
  if(!trips||!trips.length) return 0;
  // 醫낅즺일異쒖옣 以?媛일理쒓렐 복귀일李얘린
  var pastTrips=trips.filter(function(t){return pd(t.end)<=TODAY;});
  if(!pastTrips.length){
    // ?꾩쭅 異쒖옣 媛일?없으면일0
    return 0;
  }
  // 媛일理쒓렐 복귀일  var lastEnd=pastTrips.reduce(function(mx,t){
    return pd(t.end)>pd(mx)?t.end:mx;
  }, pastTrips[0].end);
  var lastEndDate=pd(lastEnd);
  // ?꾩옱 異쒖옣 以묒씠硫일쒓뎅 체류 0일  var onTrip=trips.some(function(t){return TODAY>=pd(t.start)&&TODAY<=pd(t.end);});
  if(onTrip) return 0;
  // 복귀일?ㅼ쓬?좊일일ㅻ뒛源뚯?
...
// ?? 상태 蹂일var _pmFilter='all';          // 吏일?꾪꽣
var _pmSearch='';             // ?대쫫 寃일var _pmSortKey='name';        // ?뺣젹 湲곗?: name | location | americas | europe | other | korea
var _pmSortAsc=true;          // ?뺣젹 諛⑺뼢
var _pmTypeFilter={hq:true,outsource:true,tech:true,vision:true,host:true}; // ?몄썝?좏삎 泥댄겕

function setPmFilter(f){ _pmFilter=f; renderPersonTab(); }
function setPmSearch(v){
  _pmSearch=v.toLowerCase();
  renderPersonBody(); // 寃곌낵 ?곸뿭留?媛깆떊 - 寃?됱갹 DOM 嫄대뱶由ъ? ?딆쓬
}
...
// renderPersonTab : ?꾩껜 ?뚮뜑 (일泥?吏꾩엯, 吏일븘일蹂寃일?
// renderPersonBody: 寃곌낵 ?뚯씠釉붾쭔 媛깆떊 (寃?됀룹젙일룻일낇븘일蹂寃일일?寃?됱갹 IME ?좎?)
function renderPersonTab(){
  ...
  html+='<div class="pm-stat-card"><div class="pm-stat-val">'+totalPersons+'</div><div class="pm-stat-lbl">등록 ?몄썝</div><div class="pm-stat-sub">?꾩껜 출장자/div></div>';
  html+='<div class="pm-stat-card"><div class="pm-stat-val" style="color:#2176cc">'+onTripNow+'</div><div class="pm-stat-lbl">?꾩옱 異쒖옣 以?/div><div class="pm-stat-sub">?ㅻ뒛 湲곗?</div></div>';
  html+='<div class="pm-stat-card"><div class="pm-stat-val" style="color:#1a8c66">'+americasPersons+'</div><div class="pm-stat-lbl">誘몄＜ 寃쏀뿕 ?몄썝</div><div class="pm-stat-sub">B1 鍮꾩옄 저장/div></div>';
  html+='<div class="pm-stat-card"><div class="pm-stat-val" style="color:#cc8010">'+europePersons+'</div><div class="pm-stat-lbl">?좊읇 寃쏀뿕 ?몄썝</div><div class="pm-stat-sub">ESWA/WA 일/div></div>';
  html+='</div>';

  // ?? 而⑦듃濡?諛?(寃?됱갹 ?ы븿 - ?ш린일일踰덈쭔 ?앹꽦, ?댄썑 ?ъ깮일일일
  html+='<div class="pm-ctrl-bar" id="pmCtrlBar">';
  html+='<div class="pm-ctrl-group">';
  html+='<span style="font-size:11px;color:#666">?뵇</span>';
  html+='<input class="pm-search" id="pmSearchInp" type="text" placeholder="?대쫫 寃일.." autocomplete="off" oninput="setPmSearch(this.value)">';
  html+='</div>';
  html+='<div class="pm-ctrl-sep"></div>';
  html+='<div class="pm-ctrl-group">';
  html+='<span style="font-size:10px;color:#555">吏일/span>';
  ['all','current','americas','europe','other'].forEach(function(f){
    var lbl={all:'?꾩껜',current:'출장중',americas:'誘몄＜',europe:'?좊읇',other:'湲고?'}[f];
  ...
  html+='<span style="font-size:10px;color:#555">?몄썝</span>';
  ...
  html+='</div>';
  html+='<span style="font-size:10px;color:#444;margin-left:auto">濡ㅻ쭅12M: '+r12s+' ~ '+r12e+'</span>';
  html+='</div>';

  // ?? 寃곌낵 ?곸뿭 (寃일?뺣젹/저장蹂寃일일?div留?媛깆떊)
  html+='<div id="pmBody"></div>';

  wrap.innerHTML=html;
  renderPersonBody(); // 寃곌낵 梨꾩슦湲?}

// ?뺣젹 踰꾪듉 HTML 議곌컖 ?앹꽦 (而⑦듃濡ㅻ컮 일?뺣젹 踰꾪듉 ?낅뜲?댄듃일?ъ궗일
function buildSortBtnsHtml(){
  var sortBtns=[['name','이름'],['location','현 위치'],['americas','誘몄＜(12M)'],['europe','?좊읇(12M)'],['other','湲고?(12M)'],['korea','?쒓뎅체류']];
  var h='<span style="font-size:10px;color:#555">?뺣젹</span>';
...
// 寃곌낵 ?뚯씠釉붾쭔 媛깆떊 - pmCtrlBar/pmSearchInp DOM 嫄대뱶由ъ? ?딆쓬
function renderPersonBody(){
  var body=document.getElementById('pmBody');
  if(!body) return;

  // ?뺣젹 踰꾪듉 상태留일낅뜲?댄듃 (寃?됱갹怨?臾닿?)
  ...
  showRegions.forEach(function(region){
    ...
    sectionNames=sectionNames.slice().sort(function(a,b){
      ...
      var locStrA=locA.onTrip?locA.siteName:'?쒓뎅';
      var locStrB=locB.onTrip?locB.siteName:'?쒓뎅';
      var v;
  ...
  if(!anySection){
    html='<div style="padding:30px 10px;text-align:center;color:#555;font-size:13px">?대떦 議곌굔일?몄썝일?놁뒿?덈떎.</div>';
  }
  body.innerHTML=html;
}
...
function renderRegionSection(persons, nameList, region, rolling12){
  var labels={
    americas:{title:'誘몄＜ 異쒖옣',sub:'ESHD 쨌 ESMI 쨌 ESHG 쨌 MILS 쨌 ESOT 쨌 UC2 쨌 ?꾨?JV 쨌 BOSK_TN',color:'#1a8c66',icon:'#1a8c66',b1:true,  limit:180,warn1:120,warn2:150},
    europe:  {title:'?좊읇 異쒖옣',sub:'ESWA / WA',                                                  color:'#cc8010',icon:'#cc8010',b1:false, limit:180,warn1:120,warn2:150},
    other:   {title:'湲고? 吏일異쒖옣',sub:'그 외 사이트',                                          color:'#534AB7',icon:'#534AB7',b1:false, limit:180,warn1:120,warn2:150}
  };
  var L=labels[region];
  ...
  if(L.b1) html+='<span class="pm-sec-badge" style="background:#1a3a1a;border:1px solid #2a6a2a">B1 鍮꾩옄 愿由?/span>';
  html+='</div>';

  // ?? ?뚯씠釉일ㅻ뜑 (?뺣젹 踰꾪듉 ?ы븿)
  var thDay=region==='americas'?'誘몄＜ 출장자(12M)':(region==='europe'?'?좊읇 출장자(12M)':'湲고? 출장자(12M)');
  var regionSortKey=region; // americas|europe|other 洹몃?濡일ъ슜
  ...
  nameList.forEach(function(name){
    ...
    // 寃쎄퀬 ?덈꺼 (誘몄＜쨌?좊읇쨌湲고? ?숈씪 湲곗?)
    var daysCls='',daysNote='';
    if(rolling12Days>=L.warn2){daysCls='b1-alert';daysNote='일체류 二쇱쓽 ('+rolling12Days+'일';}
    else if(rolling12Days>=L.warn1){daysCls='b1-warn';daysNote='二쇱쓽 援ш컙 ('+rolling12Days+'일';}
    else{daysCls='b1-ok';}
    ...
    if(loc.onTrip){
      ...
      html+='<td><span style="font-size:11px;color:#4aaa70">?눖?눟 국내</span></td>';
    }
    ...
    // ?쒓뎅 체류일    var kd=Math.max(0,koreaDays);
    ...
    regionTrips.forEach(function(t){
      ...
      html+='<div class="pm-trip-row">'
        ...
        +'<span class="pm-trip-period">'+fmtFull(t.start)+' 일'+fmtFull(t.end)+'</span>'
        +'<span class="pm-trip-days" style="color:#ccc">'+totalD+'일
        +(r12d>0&&r12d<totalD?' <span style="color:#666;font-size:10px">(12M일'+r12d+'일</span>':'')
        +'</span>'
        +statusHtml(t.status)
        +(t.task?'<span style="font-size:10px;color:#666">'+t.task+'</span>':'')
        +'</div>';
    });
    if(!regionTrips.length) html+='<span class="pm-empty">?댁뿭 ?놁쓬</span>';
    html+='</div></td>';
    html+='</tr>';
  });

  html+='</tbody></table>';
  html+='<div class="pm-b1-note">* 異쒖옣?쇱? 濡ㅻ쭅 12媛쒖썡(?ㅻ뒛 湲곗? 理쒓렐 1일 湲곗? 체류일(以묐났 ?쒓굅). ?쒓뎅 체류?쇱? 留덉?留?복귀일?ㅼ쓬?좊일일ㅻ뒛源뚯일일쇱닔.'
    +(L.b1?' B1/B2 鍮꾩옄 李멸퀬 ?곹븳 180일일':'')+'</div>';
  html+='</div>';
  return html;
}
